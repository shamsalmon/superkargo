...
require (
	github.com/akuity/kargo v1.10.7
	github.com/akuity/kargo/api v1.10.7
...
// Kargo's own go.mod replace directives do NOT propagate to downstream modules,
// so the ones needed to build Kargo's packages are repeated here. These are all
// proxy/version references — no local Kargo checkout is required.
replace (
	// NB: required because of https://github.com/kubernetes-sigs/kustomize/issues/6014
...
// api/v1.10.7. Pin it to the v1.10.7 commit via a proxy pseudo-version so it
// resolves without a local checkout. When bumping Kargo, update this to the new
// version's commit.
replace github.com/akuity/kargo/api => github.com/akuity/kargo/api v0.0.0-20260612003012-17077e406ed3
