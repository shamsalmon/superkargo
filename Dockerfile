...
# superkargo controller (see hack/kargo-shim.sh). Because the chart uses a
...
COPY dist/linux/superkargo-controller /usr/local/bin/superkargo-controller
COPY hack/kargo-shim.sh /usr/local/bin/kargo
RUN chmod 0755 /usr/local/bin/kargo /usr/local/bin/superkargo-controller
